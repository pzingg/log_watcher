defmodule LogWatcher.AccountsTest do
  ...
  alias LogWatcher.Accounts.{User, UserToken}
  ...
  describe "get_user!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        Accounts.get_user!("01FTM3HVZTCEEH9JHJ1Q92PG6Z")
      end
    end
  ...
  describe "register_user/1" do
    test "requires names and email" do
    ...
    test "validates email and password when given" do
      {:error, changeset} =
        Accounts.register_user(%{first_name: "John", last_name: "Doe", email: "not valid"})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Accounts.change_user_registration(
          %User{},
          valid_user_attributes(first_name: "John", last_name: "Doe", email: email)
        )
  ...
  describe "update_user_email/2" do
    setup do
      ...
      {encoded_token, user_token} =
        UserToken.build_email_token(%User{user | email: email}, "change:#{user.email}")

      Repo.insert!(user_token)

      %{user: user, email: email, token: encoded_token}
    end
